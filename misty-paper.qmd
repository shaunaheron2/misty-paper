...
dialogue_stage_details <- read_csv('data/clean_data/dialogue_stage_details_20251231_0902.csv')

dialogue_stage_summary <- read_csv(
  'data/analysis_output/dialogue_stage_summary_20251212_153544.csv'
)

dialogue_participants <- read_csv(
  'data/clean_data/dialogue_participant_details_20251231_0902.csv'
) # only has tasks
...
df_flat_scores_final <- readRDS("full_dataset_with_items.rds") |>
  mutate(
    ...
    left_join(dialogue_participants, by=join_by(session_id))
...
post_table <- df_flat_scores_final |>
 # filter(poor_comms!=1) |>
  #drop_na(gender) |>
  select(group, contains('post'), contains('correct'), 63:93) |> #76
  data.frame() |>
  mutate(across(
    contains('post_trust'),
...
post_table |> 
  group_by(group) |> 
  summarise(across(starts_with('prop'), ~mean(.x, na.rm=trust_perc_post))) |> pivot_longer(contains('prop'), names_to = 'measure', values_to = 'prop') |> pivot_wider(names_from = 'group', values_from = 'prop') |>gt() |> fmt_percent(everything(), decimals=1)
```
