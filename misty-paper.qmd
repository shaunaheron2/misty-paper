...
## Funding Source
This research was supported by a grant from the IAMGOLD President’s Innovation Fund awarded to the first author (ROMEO File #: 6021966). The funder had no role in the design, execution, analysis, or interpretation of the study.
...
Interactions took place in a shared physical workspace that included the Misty-II social robot and a participant-facing computer interface [@mistyrobotics]. The interface displayed task materials, recorded participant responses, and supported progression through the game (see @fig-task1). Importantly, it did not function as a control mechanism for the robot. Instead, the robot autonomously monitored task progression and participant inputs via the interface, adapting its dialogue and behaviour accordingly.
...
The suspect grid was displayed on the interface and allowed participants to click on and grey out eliminated candidates, while all questions were posed verbally to the robot (see @fig-task1).
...
The second task involved a more open-ended reasoning challenge designed to shift the dependency structure from enforced to optional collaboration. Participants were presented with multiple technical logs through a simulated terminal interface that could be used to infer the location of the missing robot (see @fig-task2). The task was intentionally cryptic and difficult to solve within the allotted ten minutes without synthesizing information across multiple sources.
...
Simple descriptive comparisons of post-interaction trust measures indicated higher trust ratings in the responsive condition relative to the control condition across both trust scales (see @tbl-post-eligible). As indicated in @fig-post-eligible, average raw post-interaction scores on the Trust in Industrial Human-Robot Collaboration scale (TI-HRC) and Trust Perception Scale-HRI (TPS-HRI) differed by approximately 18 points (Likert 1-5 converted to 0-100 scale for easier comparison across scales) in the responsive condition compared to the control (@fig-post-eligible).
...
#| tbl-cap: "Post-Interaction Trust Outcomes and Task Accuracy by Policy in the Eligible Sample"
post_table |>
  mutate(across(
    post_trust_reliability:post_trust_feelings,
    ~ scales::rescale(.x, to = c(0, 100), from = c(1, 5))
  )) |>
  filter(exclusions != 'exclude') |>
  select(
...
post_table |>
  ...
  modify_caption("Post-Interaction Objective Measures by Policy") |>
  as_gt() |>
  gt::gtsave(filename = "images/summary_table.png")
```
...
```{r}
#| eval: false
#| label: fig-hri-posterior
#| fig-cap: "Posterior distributions of fixed effects from the final TPS-HRI model. Shaded regions indicate 80% credible intervals; dashed vertical line denotes a null effect. Positive values indicate higher post-interaction trust scores."
#| fig-align: left
# plot_title1 <- ggtitle(
#   "TPS-HRI"
# )
...
hri_plot <- mcmc_areas(
  ...
  geom_vline(xintercept = 0, linetype = "dashed") +
  scale_y_discrete(labels = labels) +
  #labs(x = "Posterior estimate of Trust (points on 0-100 trust scale)") +
  plot_title1 +
  scale_y_discrete(labels = labels) +
  theme(
    axis.title.y = element_blank(),
...
#### Mechanism Analysis: Communication Breakdown (n=29)
  ...
  To examine whether communication quality moderated the effect of interaction policy on
  trust, Bayesian models were fitted in the full sample with the proportion of turns
  characterized by communication breakdown included as an interaction term.
  @fig-mech-posterior shows predicted trust as a function of communication breakdown for
  each condition and trust measure.

  For TI-HRC (experienced trust), the responsive condition showed consistently higher trust than control at low levels of communication breakdown, but this advantage diminished as breakdown increased (interaction posterior median $\beta$ = -5.97, 95% credible interval [-23.01, 10.92]; 76% probability of a negative interaction). This pattern is consistent with the interpretation that experienced trust depends on the robot's sustained ability to deliver responsive behaviour: when communication fails frequently, the affect-adaptive features that define the responsive policy cannot be expressed, and the trust advantage over the control condition erodes. In contrast, TPS-HRI (perceived trust) showed a weak and unstable interaction centered near zero (posterior median $\beta$ = -1.14, 95% credible interval [-18.87, 16.28]; 55% probability negative), with both conditions showing broadly similar and relatively flat predicted trust across the breakdown range.

  This suggests that evaluative trust judgments are less sensitive to graded communication degradation, potentially because they are anchored by discrete moments of successful collaboration that can occur even within partially disrupted interactions.

  Together, these results clarify the sensitivity analysis findings. The attenuation of the responsive policy effect in the full sample is not simply noise from increased variability, but reflects the inclusion of sessions in which the communication channel required to deliver the responsive manipulation had substantially failed. This supports both the exclusion criterion applied in the primary analysis and the broader interpretation that experienced trust, in particular, is contingent on sustained interaction quality.

```{r}
#| label: fig-mech-posterior
#| fig-cap: "Predicted trust scores as a function of communication breakdown, stratified by interaction policy. Panels show experienced trust (left) and perceived trust (right). Lines represent posterior medians and shaded bands indicate 80% credible intervals. Communication breakdown is defined as the proportion of interaction turns flagged as breakdowns; predictions are shown at mean baseline negative attitudes toward robots."
#| fig-align: left
#| fig-width: 9
#| fig-height: 5
scores_df_full <- readRDS("scores_df_full.rds")
...
```{r}
#| eval: false
df <- scores_df_full |> filter(scale != 'HRI_perception_post')

# df = the data used to fit the model (or at least same columns/levels)
grid <- crossing(
  prop_comm_breakdown = seq(0, 1, length.out = 101),
  group = levels(df$group)
) %>%
  mutate(
    nars_pre_c = 0 # if it's centered, 0 is perfect
    # add other covariates here the same way (e.g., native_english = "Native")
  )

pred_draws <- hrc_mod_mech %>%
  add_epred_draws(newdata = grid, re_formula = NA)

plot_df <- pred_draws %>%
  group_by(group, prop_comm_breakdown) %>%
  median_qi(.epred, .width = c(0.8, 0.95))

ggplot(
  plot_df,
  aes(x = prop_comm_breakdown, y = .epred, color = group, fill = group)
) +
  geom_ribbon(aes(ymin = .lower, ymax = .upper), alpha = 0.2, color = NA) +
  geom_line() +
  labs(
    x = "Proportion communication breakdown",
    y = "Predicted trust",
    color = "Policy",
    fill = "Policy"
  ) +
...
# nars_vals <- quantile(df$nars_pre_c, probs = c(.1, .5, .9), na.rm = TRUE)

# grid <- crossing(
#   prop_comm_breakdown = seq(0, 1, length.out = 101),
#   group = levels(df$group),
#   nars_pre_c = as.numeric(nars_vals)
# )
...
# plot_df <- pred_draws %>%
#   group_by(group, nars_pre_c, prop_comm_breakdown) %>%
#   median_qi(.epred, .width = c(0.8, 0.95))
...
#   facet_wrap(
#     ~nars_pre_c,
#     labeller = label_bquote(nars_pre_c == .(round(as.numeric(nars_pre_c), 2)))
#   ) +
#   theme_classic()
```
...
post_table |>
  ...
  tbl_summary(
    ...
    label = list(
      suspect_correct ~ "Suspect ID (robot dep.)",
      # status_correct ~ "Status Accuracy",
      # duration_mins ~ "Avg Session Duration (mins)",
      # n_turns ~ "Dialogue Turns",
      ...
      # n_engaged ~ "Engaged Responses",
      # n_frust ~ "Frustrated Responses",
      building_correct ~ "Building ID",
      zone_correct ~ "Zone ID",
      floor_correct ~ "Floor ID"
      # trust_perc_post ~ "Trust Perception Scale HRI",
      # post_trust ~ "Trust in Industrial HRI Collaboration"
    )
  ) |>
...
#as_gt() |>
# cols_width(1 ~ '20%')
#gt::gtsave(filename = "images/task_table.png")
```
...
Trust in human-robot interaction is widely recognized as multidimensional, encompassing perceptions of reliability and competence as well as affective comfort and perceived collaboration [@lee2004; @devisser2020]. The use of two validated trust measures in the present study allowed these components to be examined in parallel. Rather than treating trust as a static attribute of the system, the results suggest that it functioned as an evaluation of the collaborative experience as it unfolded over time. Notably, affective and experiential components of trust were more sensitive to interaction degradation than reliability-focused evaluations, reinforcing the view that trust in autonomous HRI is shaped by process-level interaction dynamics rather than performance outcomes alone.
...
During the preparation of this work, the author used Claude Code (Sonnet 4.6, Anthropic) to review the manuscript for clarity and readability. After reviewing the AI-generated suggestions, the author incorporated suggestions and revisions to the text as appropriate. The author takes full responsibility for the content of the published work.
...
-   Task outcome variables were coded once per session_id × stage.
