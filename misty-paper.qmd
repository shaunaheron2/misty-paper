...
abstract: |
    Trust plays a central role in human-robot collaboration, yet its formation is rarely examined under the constraints of fully autonomous interaction. This pilot study investigated how interaction policy influences trust during in-person collaboration with a social robot operating without Wizard-of-Oz control or scripted repair. Participants completed a multi-stage collaborative task with a robot that autonomously managed spoken-language dialogue, affect inference, and task progression. Two interaction policies were compared: a responsive policy, in which the robot proactively adapted its dialogue and assistance based on inferred interaction state, and a neutral, reactive policy, in which the robot provided only direct, task-relevant responses when prompted. Responsive interaction was associated with significantly higher post-interaction trust on both trust instruments under viable communication conditions, despite no reliable differences in overall task accuracy. As language-mediated interaction degraded, this advantage attenuated, and ratings became less clearly interpretable as calibrated evaluations of collaborative competence. A tentative pattern in which experienced trust (TI-HRC) appeared more sensitive to communication breakdown than perceived trust (TPS-HRI) is reported descriptively as the small sample size and exploratory design preclude formal tests of differential sensitivity across trust dimensions. These preliminary findings are consistent with the interpretation that trust in autonomous human-robot interaction is shaped by process-level interaction dynamics and operates within constraints imposed by communication viability, highlighting the importance of evaluating trust under real autonomy conditions when designing interactive robotic systems.
keywords: 
...
format:
  elsevier-pdf:
    ...
    journal:
      name: Journal Name
      formatting: doubleblind #doubleblind #preprint #doubleblind
      model: 3p # Don't set a model with preprint
      graphical-abstract: "![](images/visual-abstract.png)"
      #layout: twocolumn
      cite-style: authoryear
      #cite-method: natbib
      highlights:
        - Robots that are responsive to users are rated more trustworthy than a robot that only reacts
        - Unprompted help and empathy from robots can build greater collaboration trust
...
As artificial intelligence (AI) technologies advance, they are increasingly integrated into robotic systems, enabling more adaptive, autonomous, and context-sensitive behaviour. This convergence has accelerated the deployment of robots across safety-critical domains such as manufacturing [@choi2025], mining [@fu2021], and healthcare [@ciuffreda2025], where robots are increasingly expected to operate alongside humans rather than in isolation. In these collaborative settings, successful deployment depends not only on technical performance and safety guarantees, but also on whether human users are willing to rely on, communicate with, and coordinate their actions around robotic partners [@diab2025; @spitale2023; @emaminejad2022]. Trust has therefore emerged as a central determinant of adoption and effective use in human-robot collaboration (HRC) [@wischnewski2023; @campagna2025].

In HRC, trust is commonly understood as a willingness to rely on an agent under conditions of uncertainty and risk [@muir1994]. @muir1994 suggests that reliance is dynamically calibrated, shaping how closely users monitor a robot, when they intervene, and whether they defer or override its actions. This calibration process is especially salient in dialogue-driven collaborative tasks, where misunderstandings, delays, or ambiguous responses may directly influence users' ongoing assessments of a robot's competence and reliability [@devisser2020]. Insufficient trust can lead to disuse or rejection of automation, while excessive trust risks overreliance and accidents--particularly in environments characterized by uncertainty or incomplete information [@devisser2020].

A substantial body of human-robot interaction (HRI) research has examined how robot behaviour influences user trust, perceived reliability, and cooperation across industrial and social contexts [@hancock2011]. For example, @babel2021 found that a robot's proactive and socially expressive behaviour--such as gaze and dialogue initiative--could enhance trust and acceptance, whereas faulty or unreliable behaviour reduced perceived trustworthiness, an effect that explanation and repair strategies only partly mitigated. In the literature, trust is typically conceptualized as a multidimensional construct encompassing cognitive evaluations of competence, predictability, and reliability--alongside a behavioural willingness to collaborate toward shared goals under conditions of risk or uncertainty [@diab2025; @muir1994; @devisser2020].

Empirical studies have most often operationalized trust using post-interaction self-report questionnaires collected following short, human controlled, and often scripted interactions. Behavioural indices (reliance, intervention, compliance), physiological measures, and real-time or trust-game paradigms have also been used [@loizaga2024], but a recent systematic review of 100 HRI trust studies found questionnaires and trust scales to be the dominant technique, with sensor-based behavioural and physiological measures appearing in only a handful (N = 7 and N = 2, respectively) [@campagna2025]. Such measures provide valuable global assessments of user attitudes, but used on their own--and particularly when collected after short, scripted interactions--they offer limited insight into how trust is negotiated, disrupted, and repaired during ongoing interaction [@wischnewski2023].

Studying trust as an interactional process therefore requires experimental settings in which users engage with robots that exhibit both adaptive behaviour and realistic system limitations [@campagna2025]. In such settings, trust is shaped not only by task success but by how robots handle uncertainty, errors, and misalignment during interaction [@diab2025]. Fully autonomous systems, where dialogue management and response generation occur without human intervention, provide a critical testbed for examining these dynamics.

In practice, much of the existing HRI trust literature has relied on scripted behaviours, simulated environments, or Wizard-of-Oz (WoZ) paradigms in which a human operator covertly manages the robot’s behaviour [@bettencourt2025]. While these approaches are valuable for isolating specific design factors, they obscure the interaction breakdowns and system imperfections that characterize deployed autonomous robots. Prior work has established that robot failures broadly undermine trust: behavioural errors reduce cooperation and perceived reliability [@salem2015], users can exhibit overtrust even following observable performance failures [@robinette2016], and a recent systematic review identifies hardware and software failures as consistent trust barriers across deployment contexts [@gratsjova2025]. However, a qualitatively distinct form of failure--speech-channel breakdown arising from automatic speech recognition (ASR) errors--has received comparatively little empirical attention, and this gap is not incidental. Under WoZ paradigms, spoken-language understanding is performed by a human operator who can accommodate non-native speech disfluencies and unexpected phrasing in ways that ASR pipelines cannot, effectively absorbing communication failures before they enter the interaction. As a result, speech-channel viability does not manifest as an observable variable under WoZ control, becoming empirically tractable only in systems where dialogue is managed autonomously--a capability that large language model-driven architectures have only recently made feasible [@weia]. Consequently, how ASR-driven communication failure interacts with trust dynamics and interaction policy under full autonomy in HRI remains largely unexamined.

These methodological gaps concern how trust is studied; a parallel question is which robot behaviours shape it. A range of design strategies have been proposed to support appropriate trust calibration during collaboration, including robot appearance [@nicolas2021], transparency cues [@zhang2019], explanations of system intent, adaptive feedback [@val-calvo2020], and interaction pacing [@kok2020]. Many of these approaches aim to help users form accurate expectations about a robot’s capabilities and limitations, particularly in contexts involving uncertainty or partial observability. Among these strategies, adaptive interaction behaviour--how and when a robot responds to user state and task context--has been identified as a particularly influential factor in shaping perceptions of competence, reliability, and collaboration [@shayganfar2019; @fartook2025].

Recent advances in AI, in particular Large Language Models (LLMs), have expanded the range of interaction strategies available to autonomous robotic platforms, enabling systems to move beyond fixed, scripted behaviours toward adaptive interaction policies that estimate and respond to user state and task context in real time [@zhang2023]. Improvements in spoken-language processing, dialogue state tracking, and LLM-based reasoning now make it feasible to build robots that adjust not only to what a human says, but when and how assistance is provided during collaboration [@weia]. In parallel, advances in multimodal affect inference from facial expressions, language and interaction cues have made it increasingly feasible for robots to incorporate estimates of user emotional state into interaction management [@a.v.2024; @spitale2023; @atone2022]. As a result, responsiveness in contemporary HRI is increasingly understood as a property of an underlying interaction policy, governing how a robot interprets cues, initiates support, and manages uncertainty, rather than as a surface-level social behaviour [@birnbaum2016; @shayganfar2019; @zhang2023].

From an engineering perspective, this responsiveness emerges from the coordination of multiple system components responsible for perception, interpretation, and action [@diab2025; @arkin2003]: it depends on the integration of dialogue management, state tracking, and inference mechanisms that estimate task progress, interaction uncertainty, and user affect. Proactive assistance based on this integrated state representation differs fundamentally from reactive, request-based behaviour; for example, a robot may offer clarification, encouragement, or pacing adjustments when confusion, hesitation, or frustration is inferred, rather than waiting for an explicit request for help [@birnbaum2016].

Implementing such behaviour requires autonomous systems to manage spoken-language dialogue, maintain interaction state over time, and coordinate verbal and nonverbal responses in real time, all while operating under noise, latency, and sensing uncertainty [@campagna2025]. Yet despite growing interest in responsive and affect-aware robots, this work has relied on Wizard-of-Oz control [@birnbaum2016; @shayganfar2019] or pre-programmed, rule-based behaviour [@fartook2025] rather than full autonomy. Relatively little empirical work has therefore examined how such integrated interaction systems operate under fully autonomous conditions, or how their behaviour shapes trust and collaboration when communication breakdowns cannot be externally repaired.
...
The pilot study had three primary objectives: (1) to design and evaluate the feasibility of an autonomous spoken-language interaction system with affect-responsive behaviour on a mobile robot platform; (2) to assess whether a responsive interaction policy influences post-interaction trust and collaborative experience under autonomous conditions; and (3) to explore how behavioural and interaction-level indicators align with subjective trust evaluations. Although trust itself was assessed with established post-interaction self-report instruments, the present design administers them under genuine autonomy and triangulates them with these behavioural and interaction-level indicators, addressing a limitation of global self-report collected after scripted or operator-mediated interaction.

Three features distinguish this study from the broader HRI trust literature. First, interaction was fully autonomous, exposing participants to authentic ASR errors, response latencies, and misinterpretations rather than operator-mediated approximations of them. Second, speech-channel viability varied naturally across sessions, making it an observable variable rather than one absorbed by a human operator as in WoZ paradigms. Third, affect-responsiveness was implemented as an integrated interaction policy rather than a scripted behaviour. Together, these features let us examine trust under conditions that approximate real-world deployment.

Rather than optimizing for flawless interaction, this pilot study prioritizes feasibility assessment and mechanism exploration over confirmatory hypothesis testing. The sample size is small, the interaction conditions were subject to real-world autonomy constraints, and final condition assignment was not fully randomized due to participant attrition. Accordingly, results should be interpreted against the directional predictions outlined below as exploratory evidence intended to inform the design of a larger subsequent study by evaluating feasibility and identifying technical, interactional, and methodological challenges that must be addressed when evaluating affect-responsive robots in real-world contexts.

### Theoretical Rationale and Study Predictions

The present study is grounded in a process-oriented account of trust in human-robot interaction, in which trust is understood not as a static judgment of system capability but as an evaluation that emerges from and is updated during interaction [@lee2004; @devisser2020]. From this perspective, how a robot manages dialogue, uncertainty, and user affect during collaboration should be a primary driver of trust formation, independent of task outcome. Affect-responsive interaction, characterised by proactive assistance, empathic framing, and collaborative dialogue, has been identified as a particularly influential behavioural profile in shaping perceptions of competence, empathy, and collaboration quality in prior HRI research [@birnbaum2016; @shayganfar2019; @fartook2025]. Building on this theoretical basis, the present study was guided by two directional predictions and two theoretically motivated individual-difference variables.

**P1: Responsive interaction and trust.** Participants interacting with the responsive robot were expected to report higher post-interaction trust than those in the neutral, reactive condition. This prediction follows directly from the affect-responsive trust effects outlined above.

Because trust is multifaceted, post-interaction trust was assessed with two complementary instruments capturing distinct facets of the construct. Experienced trust during the interaction, including a relational, affective component, was measured with the Trust in Industrial Human–Robot Collaboration scale (TI-HRC) [@charalambous2016], while perceived trustworthiness, based on attributions of the robot's reliability and competence, was measured with the Trust Perception Scale-HRI (TPS-HRI) [@schaefer2016]. P1 was expected to hold convergently across both instruments. Given that the manipulation operates through proactive, empathic behaviour, its influence was anticipated to register most clearly on the experiential, affective dimension. The two measures were nonetheless treated as convergent indicators of the same predicted advantage rather than as a basis for formally contrasting trust dimensions--a comparison the present sample is underpowered to support.

**P2: Responsive interaction and collaborative performance.** Participants in the responsive condition were expected to show a performance advantage on all tasks. This prediction follows from research suggesting that affective cues and proactive support promote mutual grounding and smoother task coordination [@emaminejad2022; @lin2022]. Critically, this effect was not expected to be contingent on task structure: enforced collaboration tasks, where participants must rely on the robot to progress, were expected to benefit as much from responsive interaction as optional collaboration tasks, where participants can complete the task independently regardless of robot behaviour.

Two individual-difference variables were included on theoretical grounds but were not treated as formal moderation hypotheses. Baseline negative attitudes toward robots (NARS) were expected to negatively predict post-interaction trust, consistent with prior work showing that pre-existing robot-negative dispositions suppress trust formation [@nomura; @hancock2011]. Need for Cognition (NFC) was included based on prior work linking cognitive engagement tendencies to openness toward complex autonomous agents [@cacioppo1982]; however, given the small sample and pilot design, it was examined in a correlational rather than moderation framework.

Beyond these a priori predictions, several analyses in this study are explicitly exploratory and arose from empirical observations during data collection rather than pre-specified design choices. In particular, substantial variability in spoken-language interaction viability was observed across sessions as a consequence of real-world ASR constraints rather than a planned manipulation. Sensitivity and mechanism analyses examining the role of communication breakdown were therefore post-hoc in nature and should be interpreted as hypothesis-generating observations. This distinction between a priori predictions and exploratory analyses is maintained throughout the Results and Discussion.

## Methods

This study employed a between-subjects experimental design to examine how robot interaction policy influences trust and collaboration during autonomous, in-person human-robot interaction. The sole experimental factor was the robot’s interaction policy. Participants were initially randomly assigned to conditions; however, final condition allocation was partially constrained by participant attrition and scheduling, as described below.

Throughout this paper, references to “the robot” denote the autonomous interactive system comprising the Misty-II hardware platform and an integrated offboard software pipeline [@mistyrobotics]. Spoken-language understanding, dialogue management, task logic, and interaction policy execution were handled on an external edge device which interfaced with the robot via websocket interfaces. The Misty-II platform was responsible for audio capture, text to speech, and the execution of embodied behaviours including facial expressions, body movement, and LED signalling. Despite this distributed execution, all interaction decisions were generated autonomously by an AI driven centralized dialogue agent responsible for coordinating spoken-language understanding, task state, and verbal and nonverbal behaviour based on the interaction policy. Autonomy in socially assistive robotics is heterogeneous, spanning non-, semi-, and fully autonomous systems with no standardized way to evaluate it [@maure]; in that literature autonomy typically denotes operating without an operator rather than the autonomous spoken-language dialogue and affect management implemented here. Under the levels-of-robot-autonomy (LORA) framework of @beer2014, which scores autonomy by the degree to which a robot senses, plans, and acts without human input, the system operated at full autonomy: perception, decision-making, and action selection during the interaction proceeded without operator intervention. More detail on the robot platform and software architecture is provided in Appendix A.
...
![Experimental procedure and analytic sample determination. NARS = Negative Attitudes Toward Robots Scale; NFC-s = Need for Cognition (short form); TI-HRC = Trust in Industrial Human–Robot Collaboration scale; TPS-HRI = Trust Perception Scale – Human–Robot Interaction; ASR = automatic speech recognition.](procedural_flowchart.svg){#fig-procedure width="90%"}
...
It should be noted that the responsive and neutral policies differed on multiple dimensions simultaneously, including dialogue content, affective framing, check-in style, the frequency and nature of proactive guidance, and the overall volume of robot-initiated interaction. These features are functionally interdependent: affect-responsive behaviour requires additional dialogue turns to express, and proactive assistance necessarily increases session duration and robot response latency relative to a purely reactive policy. As a result, the responsive condition constitutes a bundled interaction profile rather than a single isolated manipulation, and the present design cannot determine the contribution of any individual component to observed differences in trust. The study is therefore best understood as an evaluation of a coherent interaction policy--one that integrates affective responsiveness, proactive assistance, and collaborative dialogue--rather than a test of any single behavioural feature in isolation. Disentangling the contributions of specific components remains an important direction for subsequent work.
...
Participants completed a pre-session questionnaire assessing baseline characteristics, including NARS and the short form of the Need for Cognition scale (NFC-s) [@cacioppo1982; @nomura]. These measures were used to capture a baseline of individual differences that may moderate responses to robot interaction.
...
### Responsive Interaction Was Associated with Higher Trust Independent of Task Accuracy
...
plot1 <- ggbetweenstats(
  data = post_table |> filter(exclusions != 'exclude'),
  x = group,
  y = trust_perc_post,
  # type = "p",
  conf.level = 0.95,
...
### Policy Effects on Trust Were Confirmed Across Frequentist and Bayesian Models
...
#### Responsive Policy Produced Credible Increases in Both Trust Measures (n = 24)
...
### Trust Differences Were Not Attributable to Differential Task Accuracy
...
### Trust Correlated with Interaction Dynamics Rather Than Individual Differences or Task Outcomes
...
This pilot study examined how a robot's interaction policy shapes user trust during embodied, fully autonomous, spoken-language collaboration. Although autonomous operation is now more common in social-robot research, studies of trust during fully autonomous, spoken collaboration--where the robot must sustain dialogue, estimate affect and joint problem-solve in real time--remain comparatively rare. Here, all dialogue management, affect inference, task progression, and response generation were executed autonomously, without Wizard-of-Oz control, human intervention, or scripted repair.Consequently, participants encountered the system not as an idealized interaction partner but under conditions approximating real-world deployment, including the latency, translation errors, and coordination failures characteristic of autonomous systems. As a pilot, the study aimed to establish feasibility--whether the hardware, software, and models could sustain the interaction, whether the protocol could run as intended in person, and what sample size a confirmatory study would require--while also beginning to clarify the mechanisms underlying trust formation under these constraints.

Across analytic approaches, the responsive policy was associated with higher post-interaction trust than the neutral, reactive control when interaction remained viable, and this advantage arose without a corresponding difference in task accuracy. Participants did not, in other words, trust the responsive robot more because it helped them perform better--by objective task success the two conditions were comparable--but because of how the interaction itself unfolded. The robot-dependent 'Who Dunnit' task did show a descriptive accuracy advantage for the responsive condition compared to control (64% vs. 30% correct), though this did not reach significance in the present sample. Given the magnitude of the difference, this most likely reflects limited statistical power rather than a true null effect. Taken together, the pattern indicates that trust was shaped more by the quality and tenor of the interaction process than by instrumental success alone. This is contrary to our central prediction that responsive interaction would raise trust (P1) across tasks, but is directionally consistent on the task requiring sustained collaboration with the robot (P2). This accords with accounts that treat trust as an emergent property of interaction dynamics rather than a static attribute of the system or a simple function of performance outcomes [@lee2004; @devisser2020].

Several mechanisms could account for why responsiveness was associated with higher trust even when it did not improve task accuracy. From a social-presence perspective, proactive engagement cues -- unsolicited offers of assistance, empathic acknowledgment of the participant's state, and a collaborative dialogue orientation -- signal that the robot is attending to the user as a social partner rather than merely executing a task [@birnbaum2016; @shayganfar2019]. This perceived attentiveness has been linked to affective judgments of trustworthiness and benevolence independent of task outcome [@birnbaum2016]. A social-exchange account offers a complementary reading: unprompted help functions as an investment in the collaboration, and participants may have reciprocated by extending trust, attributing cooperative intent to a robot that supported them without being asked [@fartook2025]. Proactive dialogue further supports the mutual grounding through which participants come to feel understood and coordinated with the robot; when the robot appears to track both the task state and user states as well as the user's engagement, it may reinforce competence-based trust (the robot knows what it is doing) and rapport-based trust (the robot is working with me; the robot is on my side) simultaneously. In practical terms, then, responsiveness fostered trust not through any single gesture but as a sustained property of the interaction -- one that continually updated participants' read on the robot's intent, capability, and willingness to collaborate. This is the process-level operation that P1 anticipates: trust rising with the quality of responsive interaction rather than with task success per se.

Examining the two trust instruments separately helps clarify which aspect of trust responsiveness actually moved. The scales were chosen to tap different facets: the TI-HRC indexes *experienced* trust -- how comfortable and at ease the robot made participants feel during the interaction (for example, whether its movement or proximity left them uneasy) -- whereas the TPS-HRI indexes *perceived* trustworthiness, participants' appraisals of the robot's reliability and competence. Rather than behaving as a static attribute of the system, trust on both measures tracked the collaborative experience as it unfolded, and both showed the responsive advantage under viable interaction -- convergent support for P1 across instruments. One pattern is worth noting tentatively: experienced trust (TI-HRC) appeared somewhat more sensitive to interaction degradation than perceived trustworthiness (TPS-HRI). This would be consistent with the conceptual distinction between the two instruments: experienced trust depends on the interaction actually feeling collaborative from moment to moment, which never takes hold when the robot cannot understand what the participant is saying, whereas appraisals of the robot's underlying reliability and competence appear more insulated from that breakdown. We do not, however, test this difference formally, as the present sample is underpowered and lacks a validated multidimensional measurement model.

It should be noted that the responsive advantage was not uniform: it depended on whether spoken communication ever got off the ground, and the way it failed is itself informative about the underlying mechanism. In the sessions where speech recognition faltered, conversational grounding was never established in the first place -- rather than a partnership breaking down, the interaction simply never connected, with the robot repeatedly signalling non-understanding ("My apologies, I'm sure you frustrated, but still I didn't get that -- could you repeat?", rephrased many times over) while the participant, in most cases, pressed on and completed what they could without the robot's help. As these failures accumulated, the trust benefit of responsiveness fell away. This moderation follows from what a dialogue-dependent affect-responsive policy actually requires in order to work: an intact dialogue channel through which to read the user's state, infer affect, and time proactive responses to the moment. When that channel degrades, affect inference and task-state tracking fail together, severing the feedback loop that responsiveness runs on. The policy kept doing what it was designed to do -- offering help, acknowledging feeling, initiating engagement -- but now without an accurate read on the person, those outputs may have actually increased frustration. Proactive help or empathic framing offered in response to misheard input can come across as incoherent or irritating, which may deepen the sense of breakdown rather than ease it. The neutral, reactive policy, which asks less of the speech channel and volunteers fewer unprompted responses, degrades more quietly. This asymmetry offers an explanation for why the trust gap closed specifically under communication failure: the two conditions did not simply decline in parallel; rather, the responsive policy was missing the very channel it needs to express itself, while the neutral policy's more restrained style remained serviceable or in the very least: less irritating. 

Affect-responsive policies thus operate within a communication viability window: below some threshold of intelligibility, the adaptive machinery cannot do its job, and its characteristic behaviours shift from a source of trust to a source of coordination burden. The design implication is concrete: systems built on affect-responsive policies need explicit monitoring of communication quality, backup strategies should translation fail, and more graceful-degradation strategies -- falling back to simpler, more restrained behaviour or other tools like a language model that matches the speaker's dialect when intelligibility drops -- to manage this failure mode in deployment. These observations come from the post-hoc communication-breakdown analyses rather than a direct test of P2, and so are best read as hypothesis-generating; even so, they offer a coherent mechanistic account of why dialogue continuity is a precondition for the trust benefits of responsiveness.

That these dynamics were observable at all is a consequence of the study's full autonomy. Because no experimenter intervened to repair failures, communication viability was not a nuisance variable to be controlled but an explicit, measurable feature of each interaction -- one that reflects the variability a deployed system would itself encounter. This visibility clarifies a constraint of broader relevance to the field: trust formation in spoken-language HRI presupposes a baseline of linguistic grounding. Where that grounding is absent, as in the breakdown sessions, trust ratings may reflect less a calibrated evaluation of the robot's competence or benevolence than the failure of the collaboration to take hold.

Taken together, these preliminary findings are consistent with a process-oriented account of trust in autonomous HRI, in which trust emerges from how a system manages uncertainty, repair, and coordination over time rather than from task performance alone. The study points in two directions at once: affect-responsive interaction shows real promise when communication holds, yet that promise is bounded by the system's ability to stay intelligible. In practical terms, the next gains may come less from making robots more responsive than from making responsiveness robust -- keeping the dialogue channel intact and introducing so that adaptive behaviour has something to act on. For research, this marks out a concrete agenda: stabilising language grounding and isolating, more precisely, the mechanisms through which responsiveness shapes the collaborative experience.

### Limitations

Several limitations constrain interpretation of the present findings and point directly to areas for refinement.

The study was conducted as a pilot with a small sample size (eligible n = 24; full n = 29), which limits statistical power for detecting effects, particularly higher-order interactions involving task structure, communication quality, and individual differences. Cell sizes were also unequal, with n = 10 participants in the responsive condition and n = 14 in the neutral condition among eligible sessions, reducing precision for between-group comparisons and widening credible intervals in Bayesian estimation. Although effect sizes were often substantial and broadly consistent across frequentist and Bayesian approaches, uncertainty remains high throughout. The study should be treated as generating directional evidence rather than definitive estimates, and replication with larger, balanced samples will be necessary to establish the stability of these effects and to formally test proposed mediation pathways.

Condition assignment was partially compromised by participant attrition during recruitment. Participants were initially assigned to experimental conditions using an automated randomization procedure at the point of online sign-up. However, several assigned participants did not attend their scheduled in-person session; replacement participants were then assigned to the next available slot without re-randomization, resulting in a semi-random rather than strictly randomized final allocation. Randomization checks comparing groups on all measured baseline characteristics -- including NARS, NFC, age, gender, and prior robot experience -- revealed no systematic differences, providing some evidence against gross confounding (see Table 1). Nevertheless, randomization checks assess only observed covariates. Residual confounding from unmeasured characteristics -- such as general comfort with ambiguity, verbal fluency in structured conversational tasks, or domain-specific motivation -- cannot be excluded, and causal interpretation of observed group differences should be qualified accordingly.

The decision to exclude sessions with severe communication breakdown from primary analyses introduces a potential source of selection bias that warrants explicit acknowledgment. Exclusion was based on a pre-specified dialogue viability criterion (more than 60% of turns affected by ASR failure), applied consistently and independently of outcome values. However, communication viability was not fully independent of participant characteristics -- excluded sessions were exclusively among participants whose spoken English was insufficiently recognized by the ASR pipeline during in-person interaction. If these participants differed systematically from retained participants on characteristics associated with trust outcomes -- for example, if the additional cognitive demand of working around communication failures also increased wariness toward the robot -- the effect estimates in the primary eligible-sample analyses could be upwardly biased. This risk is partially mitigated by the full-sample sensitivity analyses and the mechanism-focused analyses examining excluded sessions on interaction-process metrics; however, these approaches are insufficient to fully resolve the selection bias concern. 

A related limitation concerns spoken-language screening. Although spoken English fluency was a self-reported eligibility criterion, functional spoken-language proficiency varied substantially during in-person sessions, and in a subset of interactions persistent speech-recognition failure prevented the manipulation from operating as intended, leading to exclusion on methodological grounds. As argued in the Discussion, this reflects a genuine property of autonomous spoken-language HRI rather than a flaw specific to this design: absent a human to mask breakdowns, a minimum level of linguistic grounding becomes a precondition for the constructs of interest to be instantiated at all. More inclusive ASR pre-screening, and interaction policies and systems that detect and adapt to emerging language mismatch, are accordingly priorities for future work (see Future Directions).
...
These results directly inform the next phase of work at both empirical and systems levels. Larger, balanced samples will enable more precise estimation of effects and formal testing of mediation pathways linking responsiveness, interaction fluency, affective engagement, and differentiated trust components. Designs that better stabilise language handling will allow responsiveness to be evaluated under more controlled grounding conditions.

Architecturally, future iterations will move toward more explicit state-based or graph-oriented dialogue management, allowing conditional stage transitions, structured repair strategies, and flexible tool integration. Adaptive language-management mechanisms will be explored to detect emerging communication mismatch -- such as repeated recognition failures or repair loops -- and to adjust linguistic complexity, pacing, or response format accordingly, rather than assuming linguistic competence as a fixed prerequisite. These refinements are intended to expand the range of conditions under which affect-responsive policies can operate effectively.

## Conclusion

This pilot study provides preliminary evidence that affect-responsive interaction policies can shape trust during fully autonomous, in-person human-robot collaboration. The observed effects emerged independently of overall task accuracy and were bounded by communication viability: where linguistic grounding failed, differences between policies diminished and trust ratings became less interpretable as calibrated evaluations. Communication viability thus appears to function as a precondition for trust formation in spoken-language HRI rather than merely as a moderating variable.
...
#### Software Architecture and Dialogue Management

All system components were implemented in Python (version 3.10) [@python], integrating robot control, speech processing, dialogue management, task logic, and interaction logging into a single autonomous pipeline. Full dependency details are available in the project repository (see Data and Code Availability).

Dialogue was managed using the LangChain framework, which provided abstraction over message handling, memory persistence, and large language model integration [@Chase_LangChain_2022]. The system used Google’s Gemini 2.5 Flash Lite model as the underlying language model, selected for its low-latency response characteristics and configured to produce structured JSON-formatted outputs for reliable downstream parsing and robot execution [@gemini]. Conversation history was maintained using a buffer-based memory mechanism, resetting between participants.

System prompts were constructed dynamically at each dialogue turn, incorporating task rules, role constraints, accumulated conversation history, and structured contextual variables including current task stage, detected emotion, and timer state. This allowed the language model to access real-time interaction state without embedding control information into conversational text.
...
![A selection of facial expressions the language model could choose from each turn. Retrieved from \[1\] N. T. White, B. Cagiltay, J. E. Michaelis, and B. Mutlu, “Designing Emotionally Expressive Social Commentary to Facilitate Child-Robot Interaction,” in Interaction Design and Children, Athens Greece: ACM, Jun. 2021, pp. 314–325. doi: 10.1145/3459990.3460714.](images/misty-expressions.jpg){#fig-expressions fig-align="center" width="90%"}
...
Task information and participant responses were presented through a web-based dashboard. The dashboard displayed suspect grids, system logs, and response input fields, and communicated task progression events back to the dialogue agent via a local server interface.

### Data Collection and Logging

All interaction data were logged to a structured relational database. Logged fields included session metadata, turn-level dialogue transcripts, language model responses, nonverbal behaviour selections, response latencies, task submissions, detected emotions, and system events such as stage transitions and timer expirations. This structure supported post-hoc analysis of interaction dynamics, communication failures, and trust-relevant behaviours.

### Interaction Dynamics and Control Policy

Two interaction policies were implemented and toggled programmatically prior to each session. In the responsive mode, the robot proactively offered assistance, adjusted its dialogue based on inferred affect, and produced supportive backchannel behaviours. In the control mode, the robot provided general guidance and responded to requests only when explicitly prompted. Silence handling was implemented using a fixed threshold, after which the robot issued a check-in prompt; phrasing differed across conditions to reflect proactive versus reactive interaction strategies.
