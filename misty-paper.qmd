...
[also mention the pre-session questionnaire with NARS and NFC to capture baseline attitudes and individual differences that may moderate trust responses]
...
Fix H3--we didn't actually test this directly
...
Participants (n = 29) were recruited from the Laurentian University community through word of mouth and the SONA participant recruitment system. Initial sign-up was conducted online via Qualtrics where interested parties were 1) screened to be adults (18 years or older), fluent in written and spoken English and with normal or corrected-to-normal hearing and vision; 2) if eligible, provided informed consent; 3) given options to sign up for an in-person session; 4) administered a pre-session questionnaire that included basic demographics questions as well as Negative Attitudes Towards Robots (NARS) scale and the short Need for Cognition scale (NFC-s). In person sessions were conducted between November __ and November __ at Laurentian, with a single supplementary session conducted on December 15. 

Participants received a $15.00 gift card as compensation for their time. All procedures were approved by the university’s Research Ethics Board (add code etc here). The Misty-II robot used in this study was purchased through grant funding from the IAMGOLD President’s Innovation Fund. Sample characteristics are summarized in @tbl-pre.
...
The game consisted of five sequential stages designed to elicit interaction under differing collaboration and dependency conditions, following established approaches in HRI task design [@lin2022]. Participants could move to the next stage by clicking buttons on their dashboard as directed by the robot or at their own volition. Total session duration was approximately 15 minutes.
