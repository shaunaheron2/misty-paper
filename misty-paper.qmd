...
labels <- c(
  b_groupRESPONSIVE = "Responsive Policy",
...
hrc_plot <- ggplot(draws_long, aes(x = value, y = param)) +
  ...
  scale_y_discrete(labels = labels)+xlim(-25,30) +plot_title2
...
#| fig-cap: "Posterior distributions of fixed effects from the final Bayesian mixed-effects models (eligible sample) predicting post-interaction trust. Half-eye densities show posterior distributions; points indicate posterior medians; thick and thin intervals denote 80% and 95% credible intervals, respectively. The dashed vertical line marks a null effect, and positive values indicate higher trust scores on the respective scale (points on a 0–100 scale)."
#| fig-align: left
#| fig-width: 8
#| fig-height: 5
library(patchwork)
...
(hrc_plot | hri_plot)+ 
  plot_annotation(title = 'Eligible Sample: Posterior Distribution of Post-Interaction Trust',
  subtitle='Experienced Trust (TI-HRC) and Perceived Trust (TPS-HRI)', caption = 'test',  theme = theme(plot.title = element_text(size = 14)))
