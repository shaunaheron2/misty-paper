...
post_table <- df_flat_scores_final |>
  filter(exclusions == 'include') |>
  #drop_na(gender) |>
  select(group, contains('post'), contains('correct'), 63:90) |>
  data.frame() |>
  mutate(across(
    contains('post_trust'),
...
# mod_dep3 <- lmer(
#   robot_trust_post ~ group  + scale + (1 | session_id) + (1 | trust_items),
#   data = scores_df )#> filter(scale == 'HRI_perception_post')

anova(mod_dep, mod_dep2, mod_dep3, mod_dep4)
tab_model(mod_dep, mod_dep2)
```
...
mod_dep_b2 <- lmer(
  robot_trust_post ~ group * max_comm_breakdown + (1 | session_id),
  data = scores_df |> filter(scale == 'HRI_perception_post')
)
...
mod_dep2 <- lmer(
  robot_trust_post ~ group * poor_comms + (1 | session_id),
  data = scores_df |> filter(scale == 'HRI_perception_post')
)
mod_dep3 <- lmer(
  robot_trust_post ~ group *
...
nova(mod_dep, mod_dep2, mod_dep3, mod_dep4)
```
